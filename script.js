                    ...
                    <div className="main-content" style={{paddingTop: '7rem'}}>
                        ...
                        {currentStep && (
                            <div className="status-bar" style={{
                                ...
                                fontSize: '1.2rem',
                                fontWeight: 'bold',
                                background: currentStep.isAccepted ? 'linear-gradient(135deg, #4CAF50, #45a049)' : 
                                            currentStep.op === 'reject' ? 'linear-gradient(135deg, #f44336, #da190b)' : 
                                            'linear-gradient(135deg, #2196F3, #1976D2)',
                                color: 'white',
                                boxShadow: '0 4px 6px rgba(0,0,0,0.1)'
                            }}>
                                {currentStep.isAccepted ? '✅ ACCEPTED' : currentStep.op === 'reject' ? '❌ REJECTED' : '🔄 SIMULATING...'}
                                {currentStep.isAccepted && (
                                    <div style={{fontSize: '0.8rem', marginTop: '0.5rem', opacity: 0.9}}>
                                        Input string is valid for this PDA
                                    </div>
                                )}
                                {currentStep.op === 'reject' && (
                                    <div style={{fontSize: '0.8rem', marginTop: '0.5rem', opacity: 0.9}}>
                                        {currentStep.desc || 'Invalid input string'}
                                    </div>
                                )}
                            </div>
                        )}
